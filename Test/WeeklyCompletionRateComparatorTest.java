...
class WeeklyCompletionRateComparatorTest {
    ...
    void testHigherCompletionRateFirst() {
        ...
        Assertions.assertTrue(comparator.compare(habitMedium, habitHigh) > 0);
        Assertions.assertTrue(comparator.compare(habitMedium, habitLow) < 0);
    }

    @Test
    void testLowerCompletionRateFirst() {
        // Test that lower completion rate comes after higher rate
        Assertions.assertTrue(comparator.compare(habitLow, habitMedium) > 0);
        Assertions.assertTrue(comparator.compare(habitMedium, habitHigh) > 0);
    }
    ...
    void testEqualCompletionRate() {
        ...
        Assertions.assertEquals(0, comparator.compare(habitHigh, habitEqualHigh));
    }

    @Test
    void testComparisonConsistency() {
        // Test consistency of comparisons
        Assertions.assertEquals(-comparator.compare(habitLow, habitHigh), comparator.compare(habitHigh, habitLow));
    }

    @Test
    void testNullHabitComparison() {
        // Test that comparing with null throws an exception
        Assertions.assertThrows(NullPointerException.class, () -> comparator.compare(habitHigh, null));
    }
}
